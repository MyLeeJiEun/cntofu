...
<html lang="zh-CN">
<head>
<title>代码测试、调试与优化-C 语言编程透视</title>
<meta content='代码测试、调试与优化,C 语言编程透视' name='keywords'>
<meta content='代码测试、调试与优化,C 语言编程透视' name='description'>
...
<h1 id="代码测试调试与优化">代码测试、调试与优化</h1>
<ul>
<li><a href="#toc_7140_15195_1">前言</a></li>
<li><a href="#toc_7140_15195_2">代码测试</a></li>
...
<li><a href="#toc_7140_15195_10">代码调试</a></li>
<li><a href="#toc_7140_15195_11">静态调试：printf + gcc -D（打印程序中的变量）</a></li>
<li><a href="#toc_7140_15195_12">交互式的调试（动态调试）：gdb（支持本地和远程）/ald（汇编指令级别的调试）</a></li>
...
<h2 id="前言">前言</h2>
<p>代码写完以后往往要做测试（或验证）、调试，可能还要优化。</p>
<ul>
<li> <p>关于测试（或验证）</p> <p>通常对应着两个英文单词 <code>Verification</code> 和 <code>Validation</code>，在资料 <a href="http://satc.gsfc.nasa.gov/assure/agbsec5.txt">[1]</a> 中有关于这个的定义和一些深入的讨论，在资料 <a href="http://www.faqs.org/qa/qa-9060.html">[2]</a> 中，很多人给出了自己的看法。但是正如资料 <a href="http://www.faqs.org/qa/qa-9060.html">[2]</a> 提到的：</p>
<blockquote>
...
</blockquote> <p>所以，无论测试（或验证）目的都是为了让软件的功能能够达到需求。测试和验证通常会通过一些形式化（貌似可以简单地认为有数学根据的）或者非形式化的方法去验证程序的功能是否达到要求。</p> </li>
<li> <p>关于调试</p> <p>而调试对应英文 debug，debug 叫“驱除害虫”，也许一个软件的功能达到了要求，但是可能会在测试或者是正常运行时出现异常，因此需要处理它们。</p> </li>
<li> <p>关于优化</p> <p>debug 是为了保证程序的正确性，之后就需要考虑程序的执行效率，对于存储资源受限的嵌入式系统，程序的大小也可能是优化的对象。</p> <p>很多理论性的东西实在没有研究过，暂且不说吧。这里只是想把一些需要动手实践的东西先且记录和总结一下，另外很多工具在这里都有提到和罗列，包括 Linux 内核调试相关的方法和工具。关于更详细更深入的内容还是建议直接看后面的参考资料为妙。</p> </li>
</ul>
<p>下面的所有演示在如下环境下进行：</p>
<pre><code>$ uname -a
...
<p><span id="toc_7140_15195_2"></span></p>
<h2 id="代码测试">代码测试</h2>
<p>代码测试有很多方面，例如运行时间、函数调用关系图、代码覆盖度、性能分析（Profiling）、内存访问越界（Segmentation Fault）、缓冲区溢出（Stack Smashing 合法地进行非法的内存访问？所以很危险）、内存泄露（Memory Leak）等。</p>
<p><span id="toc_7140_15195_3"></span></p>
<h3 id="测试程序的运行时间-time">测试程序的运行时间 time</h3>
<p>Shell 提供了内置命令 <code>time</code> 用于测试程序的执行时间，默认显示结果包括三部分：实际花费时间（real time）、用户空间花费时间（user time）和内核空间花费时间（kernel time）。</p>
...
<p><code>time</code> 命令给出了程序本身的运行时间。这个测试原理非常简单，就是在程序运行（通过 <code>system</code> 函数执行）前后记录了系统时间（用 <code>times</code> 函数），然后进行求差就可以。如果程序运行时间很短，运行一次看不到效果，可以考虑采用测试纸片厚度的方法进行测试，类似把很多纸张叠到一起来测试纸张厚度一样，我们可以让程序运行很多次。</p>
...
<h3 id="函数调用关系图-calltree">函数调用关系图 calltree</h3>
<p><code>calltree</code> 可以非常简单方便地反应一个项目的函数调用关系图，虽然诸如 <code>gprof</code> 这样的工具也能做到，不过如果仅仅要得到函数调用图，<code>calltree</code> 应该是更好的选择。如果要产生图形化的输出可以使用它的 <code>-dot</code> 参数。从<a href="ftp://ftp.berlios.de/pub/calltree/calltree-2.3.tar.bz2">这里</a>可以下载到它。</p>
<p>这里是一份基本用法演示结果：</p>
...
<p>这样一份结果对于“反向工程”应该会很有帮助，它能够呈现一个程序的大体结构，对于阅读和分析源代码来说是一个非常好的选择。虽然 <code>cscope</code> 和 <code>ctags</code> 也能够提供一个函数调用的“即时”（在编辑 Vim 的过程中进行调用）视图（view），但是 <code>calltree</code> 却给了我们一个宏观的视图。</p>
...
<h3 id="性能测试工具-gprof--kprof">性能测试工具 gprof &amp; kprof</h3>
<p>参考资料<a href="http://www.linuxjournal.com/article/6758">[3]</a>详细介绍了这个工具的用法，这里仅挑选其中一个例子来演示。<code>gprof</code> 是一个命令行的工具，而 KDE 桌面环境下的 <code>kprof</code> 则给出了图形化的输出，这里仅演示前者。</p>
<p>首先来看一段代码（来自资料<a href="http://www.linuxjournal.com/article/6758">[3]</a>），算 <code>Fibonacci</code> 数列的，</p>
...
<p>通过 <code>calltree</code> 看看这段代码的视图，</p>
<pre><code>$ calltree -b -np -m *.c
main:
|   fibonacci
|   |   fibonacci ....
...
<p>可以看出程序主要涉及到一个 <code>fibonacci</code> 函数，这个函数递归调用自己。为了能够使用 <code>gprof</code>，需要编译时加上 <code>-pg</code> 选项，让 <code>Gcc</code> 加入相应的调试信息以便 <code>gprof</code> 能够产生函数执行情况的报告。</p>
...
<p>上面仅仅选取了部分执行结果，程序运行了 1 分多钟，代码运行以后产生了一个 <code>gmon.out</code> 文件，这个文件可以用于 <code>gprof</code> 产生一个相关的性能报告。</p>
...
<p>从这份结果中可观察到程序中每个函数的执行次数等情况，从而找出值得修改的函数。在对某些部分修改之后，可以再次比较程序运行时间，查看优化结果。另外，这份结果还包含一个特别有用的东西，那就是程序的动态函数调用情况，即程序运行过程中实际执行过的函数，这和 <code>calltree</code> 产生的静态调用树有所不同，它能够反应程序在该次执行过程中的函数调用情况。而如果想反应程序运行的某一时刻调用过的函数，可以考虑采用 <code>gdb</code> 的 <code>backtrace</code> 命令。</p>
<p>类似测试纸片厚度的方法，<code>gprof</code> 也提供了一个统计选项，用于对程序的多次运行结果进行统计。另外，<code>gprof</code> 有一个 KDE 下图形化接口 <code>kprof</code>，这两部分请参考资料<a href="http://www.linuxjournal.com/article/6758">[3]</a>。</p>
<p>对于非 KDE 环境，可以使用 <a href="https://code.google.com/p/jrfonseca/wiki/Gprof2Dot">Gprof2Dot</a> 把 <code>gprof</code> 输出转换成图形化结果。</p>
<p>关于 <code>dot</code> 格式的输出，也可以可以考虑通过 <code>dot</code> 命令把结果转成 <code>jpg</code> 等格式，例如：</p>
<pre><code>$ dot -Tjpg test.dot -o test.jp
</code></pre>
<p><code>gprof</code> 虽然给出了函数级别的执行情况，但是如果想关心具体哪些条件分支被执行到，哪些语句没有被执行，该怎么办？</p>
<p><span id="toc_7140_15195_6"></span></p>
<h3 id="代码覆盖率测试-gcov--ggcov">代码覆盖率测试 gcov &amp; ggcov</h3>
<p>如果要使用 <code>gcov</code>，在编译时需要加上这两个选项 <code>-fprofile-arcs -ftest-coverage</code>，这里直接用之前的 <code>fib.c</code> 做演示。</p>
<pre><code>$ ls
fib.c
...
<p>运行程序，并通过 <code>gcov</code> 分析代码的覆盖度：</p>
<pre><code>$ ./fib
$ gcov fib.c
File 'fib.c'
...
<p>可见代码量减少了，而且执行效率会有所提高，当然，这个代码效率的提高可能还跟其他因素有关，比如 <code>Gcc</code> 还优化了一些跟平台相关的指令。</p>
...
<h3 id="内存访问越界-catchsegv-libsegfaultso">内存访问越界 catchsegv, libSegFault.so</h3>
<p>"Segmentation fault" 是很头痛的一个问题，估计“纠缠”过很多人。这里仅仅演示通过 <code>catchsegv</code> 脚本测试段错误的方法，其他方法见后面相关资料。</p>
...
<p>从结果中可以看出，代码的 <code>sprintf</code> 有问题。经过检查发现它把整数当字符串输出，对于字符串的输出，需要字符串的地址作为参数，而这里的 <code>111</code> 则刚好被解释成了字符串的地址，因此 <code>sprintf</code> 试图访问 <code>111</code> 这个地址，从而发生了非法访问内存的情况，出现 “Segmentation Fault”。</p>
<p><span id="toc_7140_15195_8"></span></p>
<h3 id="缓冲区溢出-libsafeso">缓冲区溢出 libsafe.so</h3>
<p>缓冲区溢出是堆栈溢出（Stack Smashing），通常发生在对函数内的局部变量进行赋值操作时，超出了该变量的字节长度而引起对栈内原有数据（比如 eip，ebp 等）的覆盖，从而引发内存访问越界，甚至执行非法代码，导致系统崩溃。关于缓冲区的详细原理和实例分析见<a href="02-chapter5.markdown">《缓冲区溢出与注入分析》</a>。这里仅仅演示该资料中提到的一种用于检查缓冲区溢出的方法，它同样采用动态链接的 <code>PRELOAD</code> 机制提前装载一个名叫 <code>libsafe.so</code> 的库，可以从<a href="http://www.sfr-fresh.com/linux/misc/libsafe-2.0-16.tgz">这里</a>获取它，下载后，再解压，编译，得到 <code>libsafe.so</code>，</p>
...
<p>资料<a href="http://www.ibm.com/developerworks/linux/library/l-debug/">[7]</a>分析到，如果不能够对缓冲区溢出进行有效的处理，可能会存在很多潜在的危险。虽然 <code>libsafe.so</code> 采用函数替换的方法能够进行对这类 Stack Smashing 进行一定的保护，但是无法根本解决问题，alert7 大虾在资料<a href="http://www.xfocus.net/articles/200208/423.html">[10]</a>中提出了突破它的办法，资料<a href="http://www.xfocus.net/articles/200103/78.html">11</a>11]提出了另外一种保护机制。</p>
<p><span id="toc_7140_15195_9"></span></p>
<h3 id="内存泄露-memwatch-valgrind-mtrace">内存泄露 Memwatch, Valgrind, mtrace</h3>
<p>堆栈通常会被弄在一起叫，不过这两个名词却是指进程的内存映像中的两个不同的部分，栈（Stack）用于函数的参数传递、局部变量的存储等，是系统自动分配和回收的；而堆（heap）则是用户通过 <code>malloc</code> 等方式申请而且需要用户自己通过 <code>free</code> 释放的，如果申请的内存没有释放，那么将导致内存泄露，进而可能导致堆的空间被用尽；而如果已经释放的内存再次被释放（double-free）则也会出现非法操作。如果要真正理解堆和栈的区别，需要理解进程的内存映像，请参考<a href="02-chapter5.markdown">《缓冲区溢出与注入分析》</a></p>
<p>这里演示通过 <code>Memwatch</code> 来检测程序中可能存在内存泄露，可以从<a href="http://www.linkdata.se/sourcecode.html">这里</a>下载到这个工具。 使用这个工具的方式很简单，只要把它链接（ld）到可执行文件中去，并在编译时加上两个宏开关<code>-DMEMWATCH -DMW_STDIO</code>。这里演示一个简单的例子。</p>
<pre><code>$ cat test.c 
#include &lt;stdlib.h&gt;
#include &lt;stdio.h&gt;
#include "memwatch.h"
...
<p>通过测试，可以看到有一个 512 字节的空间没有被释放，而另外 512 字节空间却被连续释放两次（double-free）。<code>Valgrind</code> 和 <code>mtrace</code> 也可以做类似的工作，请参考资料<a href="http://www.faqs.org/docs/Linux-HOWTO/Valgrind-HOWTO.html">[4]</a>，<a href="http://www.cprogramming.com/debugging/valgrind.html">[5]</a>和 <code>mtrace</code> 的手册。</p>
<p><span id="toc_7140_15195_10"></span></p>
<h2 id="代码调试">代码调试</h2>
<p>调试的方法很多，调试往往要跟踪代码的运行状态，<code>printf</code> 是最基本的办法，然后呢？静态调试方法有哪些，非交互的呢？非实时的有哪些？实时的呢？用于调试内核的方法有哪些？有哪些可以用来调试汇编代码呢？</p>
<p><span id="toc_7140_15195_11"></span></p>
<h3 id="静态调试printf--gcc--d打印程序中的变量">静态调试：printf + gcc -D（打印程序中的变量）</h3>
<p>利用 <code>Gcc</code> 的宏定义开关（<code>-D</code>）和 <code>printf</code> 函数可以跟踪程序中某个位置的状态，这个状态包括当前一些变量和寄存器的值。调试时需要用 <code>-D</code> 开关进行编译，在正式发布程序时则可把 <code>-D</code> 开关去掉。这样做比单纯用 <code>printf</code> 方便很多，它可以避免清理调试代码以及由此带来的代码误删除等问题。</p>
<pre><code>$ cat test.c
#include &lt;stdio.h&gt;
#include &lt;unistd.h&gt;
...
<p>上面演示了如何跟踪普通变量和寄存器变量的办法。跟踪寄存器变量采用了内联汇编。</p>
...
<h3 id="交互式的调试动态调试gdb支持本地和远程ald汇编指令级别的调试">交互式的调试（动态调试）：gdb（支持本地和远程）/ald（汇编指令级别的调试）</h3>
<p><span id="toc_7140_15195_13"></span></p>
<h4 id="嵌入式系统调试方法-gdbservergdb">嵌入式系统调试方法 gdbserver/gdb</h4>
<p>估计大家已经非常熟悉 GDB（Gnu DeBugger）了，所以这里并不介绍常规的 <code>gdb</code> 用法，而是介绍它的服务器／客户（<code>gdbserver/gdb</code>）调试方式。这种方式非常适合嵌入式系统的调试，为什么呢？先来看看这个：</p>
...
<p><code>gdb</code> 比 <code>gdbserver</code> 大了将近 97%，如果把整个 <code>gdb</code> 搬到存储空间受限的嵌入式系统中是很不合适的，不过仅仅 5K 左右的 <code>gdbserver</code> 即使在只有 8M Flash 卡的嵌入式系统中也都足够了。所以在嵌入式开发中，我们通常先在本地主机上交叉编译好 <code>gdbserver/gdb</code>。</p>
...
<p>然后在本地主机上启动gdb客户端链接到 <code>gdb</code> 调试服务器，（<code>gdbserver_ipaddress</code> 是目标系统的IP地址，如果目标系统不支持网络，那么可以采用串口的方式，具体看手册）</p>
<pre><code>$ gdb -q
(gdb) target remote gdbserver_ipaddress:2345
...
</code></pre>
<p>其他调试过程和普通的gdb调试过程类似。</p>
<p><span id="toc_7140_15195_14"></span></p>
<h4 id="汇编代码的调试-ald">汇编代码的调试 ald</h4>
<p>用 <code>gdb</code> 调试汇编代码貌似会比较麻烦，不过有人正是因为这个原因而开发了一个专门的汇编代码调试器，名字就叫做 <code>assembly language debugger</code>，简称 <code>ald</code>，你可以从<a href="http://ald.sourceforge.net/">这里</a>下载到。</p>
...
<p>可见 <code>ald</code> 在启动时就已经运行了被它调试的 <code>test</code> 程序，并且进入了程序的入口 <code>0x8048054</code>，紧接着单步执行时，就执行了程序的第一条指令 <code>popl ecx</code>。</p>
...
<p>需要提到的是：Linux 下的调试器包括上面的 <code>gdb</code> 和 <code>ald</code>，以及 <code>strace</code> 等都用到了 Linux 系统提供的 ptrace() 系统调用，这个调用为用户访问内存映像提供了便利，如果想自己写一个调试器或者想hack一下 <code>gdb</code> 和 <code>ald</code>，那么好好阅读资料<a href="http://linuxgazette.net/issue81/sandeep.html">12</a>和 <code>man ptrace</code> 吧。</p>
<p>如果确实需要用gdb调试汇编，可以参考：</p>
<ul>
<li><a href="http://web.cecs.pdx.edu/~bjorn/CS200/linux_tutorial/">Linux Assembly "Hello World" Tutorial, CS 200</a></li>
<li><a href="http://lab46.corning-cc.edu/Documentation-Assembly_GDB_Debugger.php">Debugging your Alpha Assembly Programs using GDB</a></li>
</ul>
<p><span id="toc_7140_15195_15"></span></p>
<h3 id="实时调试gdb-tracepoint">实时调试：gdb tracepoint</h3>
<p>对于程序状态受时间影响的程序，用上述普通的设置断点的交互式调试方法并不合适，因为这种方式将由于交互时产生的通信延迟和用户输入命令的时延而完全改变程序的行为。所以 <code>gdb</code> 提出了一种方法以便预先设置断点以及在断点处需要获取的程序状态，从而让调试器自动执行断点处的动作，获取程序的状态，从而避免在断点处出现人机交互产生时延改变程序的行为。</p>
<p>这种方法叫 <code>tracepoints</code>（对应 <code>breakpoint</code>），它在 <code>gdb</code> 的用户手册里头有详细的说明，见 <a href="https://sourceware.org/gdb/onlinedocs/gdb/Tracepoints.html">Tracepoints</a>。</p>
<p>在内核中，有实现了相应的支持，叫 <a href="http://lwn.net/Articles/538818/">KGTP</a>。</p>
<p><span id="toc_7140_15195_16"></span></p>
<h3 id="调试内核">调试内核</h3>
<p>虽然这里并不会演示如何去 hack 内核，但是相关的工具还是需要简单提到的，<a href="http://elinux.org/images/c/c6/Tools_slides.pdf">这个资料</a>列出了绝大部分用于内核调试的工具，这些对你 hack 内核应该会有帮助的。</p>
<p><span id="toc_7140_15195_17"></span></p>
<h2 id="代码优化">代码优化</h2>
<p>这部分暂时没有准备足够的素材，有待进一步完善。</p>
...
<h2 id="参考资料">参考资料</h2>
<ul>
<li><a href="http://satc.gsfc.nasa.gov/assure/agbsec5.txt">VERIFICATION AND VALIDATION</a></li>
<li><a href="http://www.faqs.org/qa/qa-9060.html">difference between verification and Validation</a></li>
<li><a href="http://www.linuxjournal.com/article/6758">Coverage Measurement and Profiling(覆盖度测量和性能测试,Gcov and Gprof)</a></li>
<li>Valgrind Usage
<ul>
<li><a href="http://www.faqs.org/docs/Linux-HOWTO/Valgrind-HOWTO.html">Valgrind HOWTO</a></li>
...
<li><a href="http://www.ibm.com/developerworks/linux/library/l-debug/">Mastering Linux debugging techniques</a></li>
<li><a href="http://arxiv.org/pdf/cs.PF/0507073.pdf">Software Performance Analysis</a></li>
<li><a href="http://dslab.lzu.edu.cn/docs/publications/runtime_debug.pdf">Runtime debugging in embedded systems</a></li>
<li><a href="http://www.xfocus.net/articles/200208/423.html">绕过libsafe的保护--覆盖_dl_lookup_versioned_symbol技术</a></li>
<li><a href="http://www.xfocus.net/articles/200103/78.html">介绍Propolice怎样保护stack-smashing的攻击</a></li>
<li>Tools Provided by System：ltrace,mtrace,strace</li>
<li><a href="http://linuxgazette.net/issue81/sandeep.html">Process Tracing Using Ptrace</a></li>
<li>Kernel Debugging Related Tools：KGDB, KGOV, KFI/KFT/Ftrace, GDB Tracepoint，UML, kdb</li>
<li>用Graphviz 可视化函数调用</li>
<li><a href="http://www.tinylab.org/explor-linux-segmentation-fault/">Linux 段错误详解</a></li>
<li>源码分析之函数调用关系绘制系列
<ul>
<li><a href="http://www.tinylab.org/callgraph-draw-the-calltree-of-c-functions/">源码分析：静态分析 C 程序函数调用关系图</a></li>
<li><a href="http://www.tinylab.org/source-code-analysis-gprof2dot-draw-a-runtime-function-calls-the-c-program/">源码分析：动态分析 C 程序函数调用关系</a></li>
<li><a href="http://www.tinylab.org/source-code-analysis-dynamic-analysis-of-linux-kernel-function-calls/">源码分析：动态分析 Linux 内核函数调用关系</a></li>
<li><a href="http://www.tinylab.org/source-code-analysis-how-best-to-draw-a-function-call/">源码分析：函数调用关系绘制方法与逆向建模</a></li>
</ul> </li>
...
<script type="text/javascript">var bookId =43;var bookPageId =2879;var bookPageRelUrl ='zh/chapters/02-chapter9.markdown';</script>
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-38429407-1"></script>
<script>window.dataLayer =window.dataLayer ||[];function gtag(){dataLayer.push(arguments);}
gtag('js',new Date());gtag('config','UA-38429407-1');</script>
<script>var _hmt =_hmt ||[];(function() {var hm =document.createElement("script");hm.src ="https://hm.baidu.com/hm.js?f28e71bd2b5dee3439448dca9f534107";var s =document.getElementsByTagName("script")[0];s.parentNode.insertBefore(hm,s);})();</script>
